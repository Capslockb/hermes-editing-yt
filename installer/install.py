...
DEFAULT_OUTPUT_DIR = Path.home() / "sora-agent-editor-output"
...
def step_install_deps(ui: UI, non_interactive: bool) -> bool:
    ...
    if not ui.confirm(f"Run `pip install -r {REQUIREMENTS_FILE.name}`?", default=True) \
...
def step_register_mcp(ui: UI, env: EnvReport, plugin_dest: Optional[Path],
    ...
    if not non_interactive:
        if not ui.confirm(f"Run `{' '.join(cmd)}`?", default=True):
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not step_install_deps(ui, args.non_interactive):
        return 3
