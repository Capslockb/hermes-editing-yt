...
const ResourceCard = ({ label, sub, href, icon }: ResourceCardProps) => (
  <motion.a
    ...
    whileHover={{ y: -4 }}
    className="bg-tertiary p-6 rounded-2xl border border-white/5 hover:border-[#915eff]/50 transition-all flex items-center gap-4 group"
  >
    <img
      src={icon}
      alt={label}
      className="w-10 h-10 object-contain opacity-80 group-hover:opacity-100"
    />
    <div className="flex-1 min-w-0">
      <div className="text-white font-bold text-[16px] group-hover:text-[#915eff] transition-colors">
        {label}
      </div>
      <div className="text-secondary text-[12px] truncate">{sub}</div>
    </div>
    <div className="text-secondary group-hover:text-[#915eff] transition-colors text-[18px]">
      ↗
    </div>
  </motion.a>
);
